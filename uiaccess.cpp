            ...
            for (bCont = Process32First(hSnapshot, &pe); bCont; bCont = Process32Next(hSnapshot, &pe))
            {
                ...
                if (hProcess)
                {
                    ...
                    if (OpenProcessToken(hProcess, TOKEN_QUERY | TOKEN_DUPLICATE, &hToken))
                    {
                        ...
                        {
                            if (GetTokenInformation(hToken, TokenSessionId, &sid, sizeof(sid), &dwRetLen) &&
                                sid == dwSessionId)
                            {
                                bFound = TRUE;
                                if (DuplicateTokenEx(hToken, dwDesiredAccess, NULL, SecurityImpersonation,
    ...
    if (CheckForUIAccess(&dwErr, &fUIAccess))
    {
        ...
        else
        {
            ...
            if (ERROR_SUCCESS == dwErr)
            {
                ...
                GetStartupInfo(&si);
                if (CreateProcessAsUser(hTokenUIAccess, NULL, GetCommandLine(), NULL, NULL, FALSE, 0, NULL, NULL, &si,
                                        &pi))
                {
